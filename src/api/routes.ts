...
apiRouter.get('/units', async (req: AuthRequest, res) => {
  try {
    ...
    if (propertyId) {
      ...
      await verifyOrgAccess(req.user!.uid, property.organization_id);
      whereClause = eq(units.property_id, propertyId);
    } else if (orgId) {
      await verifyOrgAccess(req.user!.uid, orgId);
      whereClause = eq(units.organization_id, orgId);
    } else {
      return res.status(400).json({ error: 'Must provide propertyId or orgId' });
    }

    const result = await db.query.units.findMany({ where: whereClause });
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
apiRouter.put('/units/:id', async (req: AuthRequest, res) => {
  try {
    await verifyOrgAccess(req.user!.uid, req.body.organization_id);
    const id = req.params.id as string;
    const [unit] = await db.update(units).set(req.body).where(eq(units.id, id)).returning();
    res.json(unit);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
apiRouter.get('/bookings', async (req: AuthRequest, res) => {
  try {
    ...
    const conditions = [eq(bookings.organization_id, orgId)];
    if (propertyId) {
...
apiRouter.get('/payments', async (req: AuthRequest, res) => {
  try {
    ...
    const conditions = [eq(payments.organization_id, orgId)];
    if (bookingId) {
